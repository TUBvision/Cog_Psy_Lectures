...
\only<2->{
\vspace{5mm}
\textcolor{blue}{Just noticeable difference - JND}
\begin{itemize}
 \item The smallest detectable difference between two stimuli.
\end{itemize}
}
\end{overlayarea}
\end{frame}
...
\begin{center}
\includegraphics<1-3>[width=90mm]{../../../figures/signal_pmf_0_5.png} 
\includegraphics<4>[width=90mm]{../../../figures/signal_pmf_1_5.png} 
\includegraphics<5>[width=90mm]{../../../figures/signal_pmf_3_0.png} 
\includegraphics<6>[width=90mm]{../../../figures/signal_pmf_4_5.png} 
\end{center}

\begin{itemize}
 \item<2-> according to high-threshold theory the sensory mechanism will detect the stimulus when the amount of sensory evidence exceeds a fixed internal criterion
 \item<3-> $F(x)$: probability that the threshold will be exceeded by a stimulus of intensity $x$
\end{itemize}
\end{frame}
...
\only<2->{
\begin{itemize}
 \item at each stimulus level $x$, observer responds 'yes' with certain probability
 \item probability increases with increasing $x$
 \item $F(\alpha, \beta) = \frac{1}{1+exp(-\frac{x-\alpha}{\beta})}$
 \item $F^{-1}$ $\rightarrow$ threshold
\end{itemize}
}
